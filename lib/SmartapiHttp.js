  ...
  async init() {
    try {
      ...
      this.httpClient.interceptors.response.use(
        (response) => {
          const { status, message, errorcode, data } = response.data;

          // normalize broker status (they mix boolean and string)
          ...
          if (!isSuccess) {
            ...
            throw new Error(
              `broker error [${errorcode || "NO_CODE"}]: ${
                message || "Unknown error"
              }`
            );
          }

          // ✅ always return response.data.data (like your first interceptor)
        ...
        (error) => {
          ...
          if (error.response) {
            ...
            throw new Error(
              `http response error [${errorcode || error.response.status}]: ${
                message || "Unknown error"
              }`
            );
          }
          ...
          if (error.request) {
            throw new Error(`request error: ${error.message || "no response"}`);
          }

...
export class SmartApi {
      ...
      this.httpClient.interceptors.response.use(
        ...
        (error) => {
          ...
          if (error.code === "ECONNABORTED") {
            throw new Error(
              "request error timeout: The server took too long to respond."
            );
          }
          ...
          if (error.message && error.message.includes("Network Error")) {
            throw new Error(
              "request error network: Please check your internet connection."
            );
          }

          // Axios couldn't get a response (e.g., DNS issues, server unreachable)
          if (error.request) {
            const errMessage = error.message || "unknown error";
            throw new Error(`request error ${errMessage}`);
          }

          // Server responded with an error status (4xx, 5xx)
          if (error.response) {
            const errMessage = error.response.data?.message || "unknown error";
            throw new Error(`response error ${errMessage}`);
          }

          // Fallback for other unexpected errors
          throw new Error(`error ${this.debug ? error : error.message}`);
        }
      );
   ...
   *
   * @param {Object} params
   * @param {String} params.exchange  Exchange of instrument
   * @param {String} params.symboltoken Token of instrument
   * @param {String} params.interval Timeframe of CandleData use timeframe constant Etc..
   * @param {String} params.fromdate From Date like 2023-11-16 09:15
   * @param {String} params.todate To Date like 2023-11-16 09:16
 ...
 * @property {string} variety - Order variety (NORMAL,ROBO,STOPLOSS)
